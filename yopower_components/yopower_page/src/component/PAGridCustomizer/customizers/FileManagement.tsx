...
import { _600FileManagement } from "../models/customizers/_600FileManagement";

const fileCellClassName = "page-file-cell";

export const FileManagement: React.FC<ICell> = (cell) => {
    ...
    const params = JSON.parse(cell.definition.parameters) as any;
    const readOnly = params.readOnly as boolean;
    ...
    if (!readOnly) {
        items.push({
            ...
            onClick: () => {
                setLoading(true);
                handleFileUpload(cell.context, params, cell.table, cell.id, cell.col.name, setPropsValue)
                    .finally(() => setLoading(false));
            },
        });
    }
...
async function handleFileUpload(
    ...
    fileInput.type = "file";
    fileInput.accept = params.allowedTypes.map((type: any) => `.${type}`).join(",");
    ...
    fileInput.onchange = async (event: any) => {
        ...
        if (file) {
            ...
            if (file) {
                ...
                if (!params.allowedTypes.includes(fileExtension)) {

                    // Warning invalid file type
                    context.navigation.openErrorDialog({ message: `Invalid file type.Allowed ${params.allowedTypes.join(", ")}` });
                    return null;
                }
