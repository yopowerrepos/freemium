...
export function getColors(cell: ICell): React.ReactElement | null | undefined {
	...
	switch (cell.col.dataType) {
		...
		case "FloatingPoint":
		case "Currency":
			value = cell.props.value !== null && cell.props.value !== undefined ? Number(cell.props.value) : null;
	...
	const rule = params.rules.find(r => {
		if (r.criteria === null || r.criteria === undefined || r.criteria === 'range')
			return value !== null && r.min <= value && r.max >= value;
		if (r.criteria === 'is-null')
			return value === null;
		if (r.criteria === 'is-not-null')
			return value !== null;
	}) ?? null;
